...
Please note that Azure AD Connect is designed to clean up the objects which it was responsible to provision in  Azure AD. If the system has not provisioned the object in Azure AD in the past but gets the Azure AD object during an import step, it correctly assumes that this object was created in Azure AD by some other system. Consequently, it does not clean up such Azure AD objects even when cloudFiltered is set to True.
